...
export default class DefaultDatafeed implements Datafeed {
  ...
  async searchSymbols(search?: string): Promise<SymbolInfo[]> {
    const response = await fetch(
      `https://api.polygon.io/v3/reference/tickers?market=fx&apiKey=${
        this._apiKey
  ...
  subscribe(
    ...
    if (this._prevSymbolMarket !== symbol.market) {
      this._ws?.close();
      this._ws = new WebSocket(`wss://socket.polygon.io/${symbol.market}`);
      this._ws.onopen = () => {
        this._ws?.send(
      ...
      this._ws.onmessage = (event) => {
        ...
        if (result[0].ev === "status") {
          ...
          if (result[0].ev == "CAS") {
            ...
            const rawTimestamp = result[0].s;

            // Align the timestamp to the current period boundary
            let alignedTimestamp = this.alignTimestampToPeriodBoundary(
              rawTimestamp,
              this._currentPeriod!
            );

            callback({
              timestamp: alignedTimestamp, // Use the aligned timestamp
              open: result[0].o,
              high: result[0].h,
              low: result[0].l,
              close: result[0].c,
      ...
      this._ws?.send(
        JSON.stringify({ action: "subscribe", params: `CAS.${symbol.ticker}` })
      );
    }
